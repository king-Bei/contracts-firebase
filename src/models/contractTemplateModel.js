 ...
 */
async function create({ name, content, variables }) {
  const queryText = `
  ...
  const values = [name, content, JSON.stringify(variables)];
 ...
 * 更新合約範本。
 * @param {number} id - 範本 ID。
 * @param {object} fields - 包含 name, content, variables, is_active 的物件。
 * @returns {object|null} - 更新後的範本物件或 null。
 */
async function update(id, { name, content, variables, is_active }) {
  const queryText = `
    UPDATE contract_templates
    SET name = $1, content = $2, variables = $3, is_active = $4, updated_at = CURRENT_TIMESTAMP
    WHERE id = $5
    RETURNING *;
  `;
  const values = [name, content, JSON.stringify(variables), is_active, id];
  const { rows } = await db.query(queryText, values);
  return rows[0] || null;
}
